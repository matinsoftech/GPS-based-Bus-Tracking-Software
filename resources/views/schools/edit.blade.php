                ...
                <div class="md:col-span-2">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button type="button" onclick="clearSchoolPicker()" title="Clear the picked location"
                    ...
                    <div class="relative mt-2">
                        <div class="flex gap-2">
                            <input type="text" id="schoolSearch"
                                placeholder="Search for a place, e.g. Biratnagar Airport" autocomplete="off"
                                class="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-900 focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-white">
                            <button type="button" onclick="searchSchoolLocation()"
                        ...
                        <ul id="schoolSearchSuggestions"
                            class="absolute z-20 mt-1 hidden max-h-64 w-full overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800">
                        </ul>
                    </div>
                    ...
                    <div id="schoolMap"
                        class="mt-2 h-96 min-h-[384px] w-full overflow-hidden rounded-xl border border-gray-300 bg-gray-100 z-0 dark:border-gray-700">
                    </div>
                    <p id="schoolPickerReadout" class="mt-2 space-y-0.5 text-xs">
                        ...
                        <span id="schoolPickerStatus" class="font-medium text-gray-500 dark:text-gray-400">Click the
                            map to pick a location</span>
                    </p>
                </div>
...
<script>
    (function() {
        ...
        const suggestionsEl = document.getElementById('schoolSearchSuggestions');
        ...
        function initSchoolMap(initialLat, initialLng) {
            ...
            schoolMap = L.map('schoolMap', {
                ...
                updateWhenIdle: true,
                keepBuffer: 1,
                zoomControl: false, // using our own +/- buttons instead
            ...
            if (initialLat !== null && initialLng !== null) {
                ...
                schoolMap.setView([26.4525, 87.2718], 13); // Biratnagar fallback
            }
            ...
            // Custom zoom control, bottom-right so it doesn't collide with the search box
            L.control.zoom({
        ...
        function placeSchoolMarker(lat, lng, name) {
        ...
        window.clearSchoolPicker = function() {
            if (schoolMarker) {
                schoolMap.removeLayer(schoolMarker);
                schoolMarker = null;
            }
            ...
            hideSuggestions();
            updateReadout(null, null);
            if (schoolMap) schoolMap.setView([26.4525, 87.2718], 13);
        };

        window.useMyLocationForSchool = function() {
            initSchoolMap(null, null);
            if (!schoolMap) {
            ...
            navigator.geolocation.getCurrentPosition(
                function(pos) {
                    ...
                    schoolMap.setView([pos.coords.latitude, pos.coords.longitude], 16);
                },
                function() {
                    setStatus('Location unavailable — click the map instead');
                }, {
        ...
        // --- Search: button-triggered single result (kept for Enter / Search button) ---
        window.searchSchoolLocation = async function() {
        ...
        function renderSuggestions(results) {
            ...
            suggestionsEl.innerHTML = results.map(function(r, i) {
                return '<li data-index="' + i +
                    '" class="cursor-pointer px-4 py-2 text-sm text-gray-700 hover:bg-brand-50 dark:text-gray-200 dark:hover:bg-gray-700">' +
                    r.display_name + '</li>';
            }).join('');

            suggestionsEl.classList.remove('hidden');

            Array.from(suggestionsEl.children).forEach(function(li, i) {
        ...
        function selectSuggestion(result) {
            ...
            placeSchoolMarker(lat, lng, result.display_name);
