                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse ($routes as $route)
                            <tr class="text-gray-700 dark:text-gray-200">
                                ...
                                <td class="px-5 py-3">
                                    {{ $route->estimated_duration ? $route->estimated_duration . ' min' : '—' }}</td>
            ...
            @forelse ($routes as $route)
                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            <p class="truncate font-medium text-gray-900 dark:text-white">{{ $route->name }}</p>
                            <p class="truncate text-xs text-gray-500 dark:text-gray-400">{{ $route->route_code }}</p>
                        </div>
                        @if ($route->is_active)
                            <span
                    ...
                    <dl class="mt-3 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
                        ...
                        <div>
                            ...
                            <dd class="text-gray-700 dark:text-gray-200">
                                {{ $route->estimated_distance ? $route->estimated_distance . ' km' : '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-gray-400 dark:text-gray-500">Duration</dt>
                            <dd class="text-gray-700 dark:text-gray-200">
                                {{ $route->estimated_duration ? $route->estimated_duration . ' min' : '—' }}</dd>
                        </div>
                        <div class="col-span-2">
                            <dt class="text-gray-400 dark:text-gray-500">School</dt>
                            <dd class="text-gray-700 dark:text-gray-200">{{ $route->school->name ?? '—' }}</dd>
                        </div>
                    </dl>
