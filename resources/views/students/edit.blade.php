<x-app-layout page="student-management">
    <div class="mx-auto max-w-(--breakpoint-2xl) p-4 md:p-6">
        ...
        @php
            $selectedRoutesInit = old('route_ids', $student->routes->pluck('id')->map(fn ($id) => (string) $id)->all());
            if (!is_array($selectedRoutesInit)) {
                $selectedRoutesInit = [];
            }

            $selectedStopInit = array_values(array_unique(array_merge(
                $student->stops->pluck('id')->map(fn ($id) => (int) $id)->all(),
                array_map('intval', old('stops', []))
            )));
            ...
            <div>
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div>
                            ...
                            <input type="text" value="{{ $student->school->name ?? 'School not assigned' }}" readonly
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                School cannot be changed after the student is created.
                            </p>
                        </div>
