<x-app-layout page="drivers">
    <div class="mx-auto max-w-(--breakpoint-2xl) p-4 md:p-6">
        ...
        <form action="{{ route('drivers.update', $driver) }}" method="POST" enctype="multipart/form-data"
            ...
            x-data="{
                schoolId: @js((string) (old('school_id') ?? $driver->school_id ?? '')),
                ...
                selectedBusIds: @js(collect(old('bus_ids', $driver->buses->pluck('id')->all()))->map(fn ($id) => (int) $id)->values()),
                selectedRouteIds: @js(collect(old('route_ids', $driver->routes->pluck('id')->all()))->map(fn ($id) => (int) $id)->values()),
                get filteredBuses() {
                        ...
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                            @if ($driver->profile_photo)
                                <img src="{{ asset('storage/' . $driver->profile_photo) }}"
                                    alt="{{ $driver->full_name }}" class="h-20 w-20 rounded-full object-cover">
                            @else
                                <div
                                    class="flex h-20 w-20 items-center justify-center rounded-full bg-gray-100 text-2xl font-semibold text-gray-500 dark:bg-gray-800 dark:text-gray-300">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    @if (auth()->user()->hasAnyRole(['School Admin', 'Principal']))
                        <div>
                            ...
                            <input type="text" value="{{ $driver->school->name ?? 'School not assigned' }}"
                                readonly
                        ...
                        <div>
                            ...
                            <select id="school_id" name="school_id" required x-model="schoolId"
                                ...
                                @foreach ($schools as $school)
                                    <option value="{{ $school->id }}" @selected(old('school_id', $driver->school_id) == $school->id)>
                                        {{ $school->name }}</option>
                                @endforeach
                            </select>
                            @error('school_id')
