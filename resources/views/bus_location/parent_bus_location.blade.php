<x-app-layout page="bus-location">
    <div class="mx-auto max-w-(--breakpoint-2xl) p-4 md:p-6 space-y-6">
        ...
        <div
            ...
            <div>
                <div class="flex items-center gap-3">
                    <div
                        class="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-50 text-brand-600 dark:bg-brand-500/15 dark:text-brand-400">
            ...
            @if ($children->count() > 1)
                ...
                <div class="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 custom-scrollbar">
                    ...
                    @foreach ($children as $child)
                        ...
                        @endphp
                        <a href="{{ route('bus_location', ['child_id' => $child->id]) }}"
                            ...
                            <span
                                class="flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold {{ $isSelected ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300' }}">
        ...
        @if (!$selectedChild || !$route || $routes->isEmpty())
            ...
            <div
                class="rounded-2xl border border-gray-200 bg-white p-8 md:p-12 text-center shadow-xs dark:border-gray-800 dark:bg-white/[0.03]">
                <div
                    class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400 mb-4">
                ...
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto">
                    @if ($selectedChild)
                        <strong>{{ $selectedChild->full_name }}</strong> is currently not assigned to a school bus
                        route.
                    @else
                ...
                <div class="mt-6">
                    <a href="{{ route('parent.dashboard') }}"
                        class="inline-flex items-center gap-2 rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-semibold text-white shadow-xs transition hover:bg-brand-600">
                        Back to Dashboard
                    </a>
                </div>
            </div>
        @else
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                ...
                <!-- Card 1: Bus Status & Speed -->
                <div
                    class="rounded-2xl border border-gray-200 bg-white p-5 shadow-xs dark:border-gray-800 dark:bg-white/[0.03] flex flex-col justify-between">
                    <div class="flex items-center justify-between">
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Bus Status</span>
                        <div
                    ...
                    <div class="mt-3">
                        <div class="flex items-center gap-2">
                            <span id="liveBusStatusBadge"
                                class="inline-flex items-center gap-1.5 rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-semibold text-gray-600 dark:bg-gray-800 dark:text-gray-400 border border-gray-200 dark:border-gray-700">
                                <span class="h-1.5 w-1.5 rounded-full bg-gray-400"></span>
                                Awaiting Signal
                            </span>
                        </div>
                        <p class="mt-2 text-2xl font-bold text-gray-900 dark:text-white font-mono">
                            <span id="liveBusSpeed">—</span> <span
                                class="text-xs font-normal text-gray-500 dark:text-gray-400">km/h</span>
                        </p>
                        <p class="mt-1 text-[11px] text-gray-400 dark:text-gray-500">
                            Last updated: <span id="liveLastUpdateText">—</span>
                        </p>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div
                            class="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="mt-3">
                        <p id="liveNextStopCardText"
                            class="text-sm font-bold text-brand-600 dark:text-brand-400 truncate">
                        ...
                        <p class="mt-1 text-[11px] text-gray-400 dark:text-gray-500 flex items-center justify-between">
                            <span>Distance:</span>
                            <span id="liveDistCardText"
                                class="font-mono font-semibold text-gray-700 dark:text-gray-300">—</span>
                        </p>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="mt-3">
                        @if ($bus)
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-bold text-gray-900 dark:text-white">Bus
                                    #{{ $bus->bus_number }}</span>
                            ...
                            <p class="mt-1 text-xs font-medium text-gray-700 dark:text-gray-300">
                                Driver: {{ $bus->drivers->first()?->full_name ?? 'Assigned Driver' }}
                            </p>
                            @if ($bus->drivers->first()?->phone)
                                <a href="tel:{{ $bus->drivers->first()->phone }}"
                ...
                <div
                    ...
                    <div class="mt-3">
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            {{ trim($selectedChild->grade . ' ' . $selectedChild->section) }} • Roll
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
                ...
                <div
                    class="xl:col-span-5 rounded-2xl border border-gray-200 bg-white p-6 shadow-xs dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">Real-time stop completion & ETAs
                                </p>
                            </div>
                        </div>

                        <span id="journeyStatusPill"
                    ...
                    <div
                        class="mb-5 rounded-xl bg-gray-50/80 p-4 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800/60">
                        <div class="flex items-center justify-between text-xs font-semibold mb-1.5">
                            <span class="text-gray-600 dark:text-gray-400">Overall Route Progress</span>
                            <span id="progressBarLabel" class="text-brand-600 dark:text-brand-400 font-mono">0%
                                Completed</span>
                        </div>
                            ...
                            @forelse ($route->stops as $index => $stop)
                                ...
                                <div id="journeyStopItem-{{ $index }}"
                                    ...
                                    <div class="relative flex flex-col items-center">
                                        ...
                                        @if (!$isLast)
                                            <div id="stopConnectorLine-{{ $index }}"
                                    ...
                                    <div id="stopCardBox-{{ $index }}"
                                        ...
                                        <div class="flex flex-wrap items-center justify-between gap-1.5">
                                            ...
                                            <div class="text-right">
                                                <div
                                                    class="text-[11px] font-mono font-semibold text-gray-700 dark:text-gray-300">
                                                    Sched:
                                                    {{ $stop->pickup_time ? date('h:i A', strtotime($stop->pickup_time)) : ($stop->drop_time ? date('h:i A', strtotime($stop->drop_time)) : '07:00 AM') }}
                                                </div>
                                                <div id="stopEtaText-{{ $index }}"
                                                    class="text-[11px] font-mono font-medium text-gray-600 dark:text-gray-400">
                                                    —</div>
                                            </div>
                                        </div>

                                        <div
                                            class="mt-1 text-[11px] text-gray-500 dark:text-gray-400 flex items-center justify-between">
                                            <span>Stop #{{ $stop->stop_order }}</span>
                                            <span id="stopDistanceText-{{ $index }}"
...
@if ($routes->isNotEmpty())
    <script>
        ...
        function updateTelemetryCards(gps) {
            ...
            if (speedEl) speedEl.innerText = (gps.speed_kmh == null) ? '—' : Math.round(gps.speed_kmh);

            const badge = document.getElementById('liveBusStatusBadge');
            if (badge) {
                badge.innerHTML = '<span class="h-1.5 w-1.5 rounded-full animate-pulse" style="background-color:' + gps
                    .status_color + '"></span>' + gps.status_label;
            ...
            if (statusPill) {
                statusPill.innerHTML = '<span class="h-2 w-2 rounded-full animate-pulse" style="background-color:' + gps
                    .status_color + '"></span>' + gps.status_label;
        ...
        function formatDurationMinutes(etaMinutes) {
            if (etaMinutes < 1) return 'under a minute';
            if (etaMinutes < 60) return Math.round(etaMinutes) + ' mins';
        ...
        function liveNearestStop(gps) {
            ...
            for (let i = 0; i < liveJourneyStops.length; i++) {
                ...
                if (stop && stop.latitude && stop.longitude && parseFloat(stop.latitude) !== 0 && parseFloat(stop
                    .longitude) !== 0) {
                    const dist = liveGpsDistanceKm(gps.latitude, gps.longitude, parseFloat(stop.latitude), parseFloat(stop
                        .longitude));
                    if (dist < distance) {
                        distance = dist;
                        idx = i;
        ...
        function liveStopEtaMinutes(gps, stop) {
            ...
            if (gps.speed_kmh <= STOPPED_SPEED_KPH) return null;
            const distKm = liveGpsDistanceKm(gps.latitude, gps.longitude, parseFloat(stop.latitude), parseFloat(stop
                .longitude));
        ...
        function setJourneyPill(label, color) {
            ...
            if (!pill) return;
            pill.innerHTML = '<span class="h-2 w-2 rounded-full animate-pulse" style="background-color:' + color +
                '"></span>' + label;
        ...
        function renderJourneyNeutral(totalStops) {
            ...
            for (let i = 0; i < totalStops; i++) {
                ...
                if (iconNode) {
                    iconNode.className =
                        'node-icon flex h-8 w-8 items-center justify-center rounded-full bg-white border-2 border-gray-300 text-gray-400 dark:bg-gray-900 dark:border-gray-700 z-10';
                    iconNode.innerHTML = '<span class="text-xs font-mono font-bold">' + (i + 1) + '</span>';
                }
                if (connector) connector.className =
                    'connector-line w-0.5 h-12 bg-gray-200 dark:bg-gray-800 transition-colors duration-500 my-1';
                if (cardBox) cardBox.className =
                    'stop-card-box flex-1 rounded-xl p-3 border bg-gray-50/40 border-gray-100 dark:bg-gray-800/20 dark:border-gray-800/50';
        ...
        function renderJourneyProgress(gps, reachedIdx, totalStops) {
            ...
            if (reachedIdx >= totalStops - 1) {
                ...
                if (anchor && next && anchor.latitude && anchor.longitude && next.latitude && next.longitude) {
                    const distToNext = liveGpsDistanceKm(gps.latitude, gps.longitude, parseFloat(next.latitude), parseFloat(
                        next.longitude));
                    const anchorToNext = liveGpsDistanceKm(parseFloat(anchor.latitude), parseFloat(anchor.longitude),
        ...
        function renderNextStopCard(gps, reachedIdx, totalStops, offRoute) {
            ...
            if (cardEta) {
                if (offRoute) {
                    ...
                    cardEta.innerText = gps.speed_kmh <= STOPPED_SPEED_KPH ? 'Stopped' : '—';
                } else {
                    cardEta.innerText = 'In ' + formatDurationMinutes(eta);
                }
            }
        ...
        function renderJourneyTimeline(gps, reachedIdx, totalStops, offRoute) {
            for (let i = 0; i < totalStops; i++) {
                ...
                if (i < reachedIdx) {
                    if (iconNode) {
                        iconNode.className =
                            'node-icon flex h-8 w-8 items-center justify-center rounded-full bg-emerald-500 border-emerald-500 text-white z-10 shadow-xs';
                        iconNode.innerHTML =
                    ...
                    if (connector) connector.className =
                        'connector-line w-0.5 h-12 bg-emerald-500 transition-colors duration-500 my-1';
                    if (cardBox) cardBox.className =
                        'stop-card-box flex-1 rounded-xl p-3 border bg-emerald-50/30 border-emerald-100 dark:bg-emerald-500/5 dark:border-emerald-800/30 opacity-75';
                    if (titleText) titleText.className =
                        'text-xs font-semibold text-gray-500 dark:text-gray-400 line-through';
                    if (badge) badge.className = 'hidden';
                    if (etaText) {
                    ...
                    if (iconNode) {
                        iconNode.className =
                            'node-icon flex h-8 w-8 items-center justify-center rounded-full bg-emerald-500 border-2 border-white text-white z-10 ring-4 ring-emerald-500/30 shadow-md';
                        iconNode.innerHTML =
                            '<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>';
                    }
                    if (connector) connector.className =
                    ...
                    if (iconNode) {
                        iconNode.className =
                            'node-icon flex h-8 w-8 items-center justify-center rounded-full bg-brand-500 border-2 border-white text-white z-10 ring-4 ring-brand-500/30 animate-bounce shadow-md';
                        iconNode.innerHTML = '<span class="text-xs">🚌</span>';
                    }
                    if (connector) connector.className =
                        'connector-line w-0.5 h-12 bg-brand-300 dark:bg-brand-800 transition-colors duration-500 my-1';
                    if (cardBox) cardBox.className =
                        'stop-card-box flex-1 rounded-xl p-3 border bg-brand-50/80 border-brand-300 dark:bg-brand-500/15 dark:border-brand-500/40 shadow-sm ring-1 ring-brand-500/20';
                    if (titleText) titleText.className = 'text-xs font-bold text-brand-700 dark:text-brand-300';
                    if (badge) {
                        badge.className =
                            'inline-flex items-center gap-1 rounded-full bg-brand-600 px-2 py-0.5 text-[10px] font-semibold text-white animate-pulse';
                    ...
                    if (etaText) {
                        etaText.className = 'text-[11px] font-mono font-medium text-gray-600 dark:text-gray-400';
                        etaText.innerText = eta == null ? 'ETA: —' : 'ETA: ' + formatDurationMinutes(eta);
                    }
                    if (distText) distText.innerText = dist == null ? '—' : dist.toFixed(1) + ' km away';
                }
            }
