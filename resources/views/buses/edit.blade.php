                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    @if(auth()->user()->hasAnyRole(['School Admin', 'Principal']))
                        <div>
                            ...
                            <input
                                type="text"
                                value="{{ $bus->school->name ?? 'School not assigned' }}"
                                readonly
                        ...
                        <div>
                            ...
                            >
                                @foreach($schools as $school)
                                    <option value="{{ $school->id }}" @selected(old('school_id', $bus->school_id) == $school->id)>{{ $school->name }}</option>
                                @endforeach
                            </select>
                            @error('school_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        @else
                            ...
                            >
                                ...
                                @foreach ($drivers as $driver)
                                    <option value="{{ $driver->id }}" @selected(old('driver_id', $bus->driver_id) == $driver->id)>{{ $driver->full_name }} ({{ $driver->employee_id }})</option>
                                @endforeach
                            </select>
                            @error('driver_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
