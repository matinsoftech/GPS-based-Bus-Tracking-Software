<x-app-layout page="overview">
    @php
        ...
        $alerts = $expiringBuses->count() + $suspendedDrivers->count() + $unassignedBuses->count();
    @endphp
    ...
    <div class="mx-auto max-w-(--breakpoint-2xl) p-4 md:p-6">
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Welcome back, {{ $user->name }}. Here is the transport overview across all schools.
                </p>
            </div>
            <span
                class="text-theme-sm inline-flex w-fit items-center gap-2 rounded-full bg-brand-50 px-3 py-1.5 font-medium text-brand-600 dark:bg-brand-500/15 dark:text-brand-400">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 xl:grid-cols-4">
            <div
                class="flex min-h-[150px] flex-col rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
            ...
            <div
                ...
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-success-50 dark:bg-success-500/15">
                    <svg class="fill-success-600 dark:fill-success-500" width="24" height="24"
            ...
            <div
                ...
                <div class="mt-5 flex items-end justify-between">
                    <div>
                        ...
                        <span class="text-theme-xs mt-1 block text-gray-400 dark:text-gray-500">{{ $activeStudents }}
                            active</span>
                    </div>
            ...
            <div
                ...
                <div class="mt-5 flex items-end justify-between">
                    <div>
                        ...
                        <span class="text-theme-xs mt-1 block text-gray-400 dark:text-gray-500">{{ $activeDrivers }}
                            active</span>
                    </div>
        ...
        <div x-data="{ activeTab: 'overview' }" class="mt-6">
            {{-- <div
                class="mb-6 flex items-center gap-1 rounded-xl border border-gray-200 bg-white p-1 dark:border-gray-800 dark:bg-white/[0.03] w-fit">
                <button @click="activeTab = 'overview'"
                    :class="activeTab === 'overview' ? 'bg-brand-500 text-white shadow-sm' :
            ...
            <div x-show="activeTab === 'overview'" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="mt-6 grid grid-cols-12 gap-4 md:gap-6">
                    <div class="col-span-12 xl:col-span-7">
                        <div
                            class="flex h-full min-h-[460px] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
                            <div class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Fleet Status</h3>
                                    <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">
                                        Breakdown of the entire bus fleet
                                    </p>
                                </div>
                                <a href="{{ route('buses.index') }}"
                                    class="text-theme-sm font-medium text-brand-500 hover:text-brand-600">View all</a>
                            </div>

                            @if ($totalBuses > 0)
                                <div
                                    class="mt-6 flex h-3 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
                                    @if ($activeBuses > 0)
                                        <div class="bg-success-500"
                                            style="width: {{ $bar($activeBuses, $totalBuses) }}%"></div>
                                    @endif
                                    @if ($maintenanceBuses > 0)
                                        <div class="bg-warning-500"
                                            style="width: {{ $bar($maintenanceBuses, $totalBuses) }}%"></div>
                                    @endif
                                    @if ($inactiveBuses > 0)
                                        <div class="bg-gray-400 dark:bg-gray-600"
                                            style="width: {{ $bar($inactiveBuses, $totalBuses) }}%"></div>
                                    @endif
                                </div>

                                <div class="mt-4 flex flex-wrap gap-6">
                                    <span
                                        class="flex items-center gap-2 text-theme-sm text-gray-500 dark:text-gray-400">
                                ...
                                <div class="mt-5 min-h-0 flex-1 overflow-auto custom-scrollbar">
                                    <table class="w-full text-left text-sm">
                                        <thead
                                            class="border-b border-gray-200 text-theme-xs text-gray-500 dark:border-gray-800 dark:text-gray-400">
                                            <tr>
                                                ...
                                                <tr class="text-gray-700 dark:text-gray-200">
                                                    ...
                                                    <td class="py-3.5">
                                                        @if ($bus->status === 'Active')
                                                            <span
                                                                class="rounded-full bg-success-50 px-2 py-0.5 text-theme-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">Active</span>
                                                        @elseif ($bus->status === 'Maintenance')
                                                            <span
                                                                class="rounded-full bg-warning-50 px-2 py-0.5 text-theme-xs font-medium text-warning-600 dark:bg-warning-500/15 dark:text-warning-500">Maintenance</span>
                                                        @else
                                                            <span
                                                                class="rounded-full bg-gray-100 px-2 py-0.5 text-theme-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">Inactive</span>
                                                        @endif
                                                    </td>
                                                    <td class="py-3.5">
                                                        @if ($bus->drivers->isNotEmpty())
                                                            <span
                                                                class="text-theme-xs inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">
                                                                {{ $bus->drivers->first()?->full_name }}
                                                            </span>
                                                        @else
                                                            <span
                                                                class="text-theme-xs text-gray-400 dark:text-gray-500">No
                                                                driver</span>
                                                        @endif
                                                    </td>
                                                    <td class="py-3.5 text-theme-xs text-gray-500 dark:text-gray-400">
                                                        {{ $bus->school?->name ?? '—' }}</td>
                                                    <td class="py-3.5 text-right text-theme-sm">{{ $bus->capacity }}
                                                    </td>
                                                </tr>
                                            @empty
                                                <tr>
                            ...
                            @else
                                <div class="py-8 text-center">
                                    <p class="text-sm text-gray-500 dark:text-gray-400">No buses registered yet.</p>
                                    <a href="{{ route('buses.create') }}"
                                        class="mt-3 inline-flex rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">
                                        Add your first bus
                                    </a>
                                </div>
                            @endif
                    ...
                    <div class="col-span-12 xl:col-span-5">
                        <div
                            class="flex h-full min-h-[460px] flex-col rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                            <div class="flex items-center justify-between">
                                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Needs Attention</h3>
                                <span
                                    class="rounded-full bg-error-50 px-2.5 py-1 text-theme-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-500">
                                    {{ $alerts }} alerts
                                </span>
                            </div>
                            ...
                            <div class="mt-5 min-h-0 flex-1 space-y-4 overflow-y-auto custom-scrollbar pr-1">
                                @forelse ($expiringBuses as $bus)
                                    <div
                                        class="flex items-start gap-3 rounded-xl border border-error-100 bg-error-50/50 p-3 dark:border-error-500/20 dark:bg-error-500/5">
                                        <span
                                            class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-error-100 text-error-600 dark:bg-error-500/15 dark:text-error-500">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <p class="text-theme-xs mt-0.5 text-gray-500 dark:text-gray-400">
                                                {{ $bus->insurance_expiry_date->format('M d, Y') }} ·
                                                {{ $bus->insurance_expiry_date->diffForHumans() }}
                                            </p>
                                        </div>
                                    </div>
                                @empty
                                    @if ($suspendedDrivers->isEmpty() && $unassignedBuses->isEmpty())
                                        <div
                                            class="rounded-xl border border-success-100 bg-success-50/50 p-3 text-theme-sm text-success-600 dark:border-success-500/20 dark:bg-success-500/5 dark:text-success-500">
                                ...
                                @forelse ($suspendedDrivers as $driver)
                                    <div
                                        class="flex items-start gap-3 rounded-xl border border-warning-100 bg-warning-50/50 p-3 dark:border-warning-500/20 dark:bg-warning-500/5">
                                        <span
                                            class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-warning-100 text-warning-600 dark:bg-warning-500/15 dark:text-warning-500">
                                            <svg class="fill-current" width="16" height="16"
                                                viewBox="0 0 16 16" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path fill-rule="evenodd" clip-rule="evenodd"
                                                    d="M8 2C7.17157 2 6.5 2.67157 6.5 3.5V4.25H9.5V3.5C9.5 2.67157 8.82843 2 8 2ZM11 4.25V3.5C11 1.84315 9.65685 0.5 8 0.5C6.34315 0.5 5 1.84315 5 3.5V4.25H4.25C3.00736 4.25 2 5.25736 2 6.5V12.5C2 13.7426 3.00736 14.75 4.25 14.75H11.75C12.9926 14.75 14 13.7426 14 12.5V6.5C14 5.25736 12.9926 4.25 11.75 4.25H11ZM3.5 6.5C3.5 6.08579 3.83579 5.75 4.25 5.75H11.75C12.1642 5.75 12.5 6.08579 12.5 6.5V12.5C12.5 12.9142 12.1642 13.25 11.75 13.25H4.25C3.83579 13.25 3.5 12.9142 3.5 12.5V6.5Z"
                                                    fill="" />
                                            </svg>
                                        </span>
                                        <div class="min-w-0 flex-1">
                                ...
                                @empty
                                    @if ($expiringBuses->isEmpty() && $unassignedBuses->isEmpty())
                                        <p class="text-theme-sm text-gray-500 dark:text-gray-400">Everything looks
                                            good.</p>
                                    @endif
                                @endforelse

                                @forelse ($unassignedBuses as $bus)
                                    <div
                                        class="flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50/50 p-3 dark:border-gray-800 dark:bg-white/[0.03]">
                                        <span
                                            class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300">
                                            <svg class="fill-current" width="16" height="16"
                                                viewBox="0 0 16 16" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <path fill-rule="evenodd" clip-rule="evenodd"
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <p class="text-theme-sm font-medium text-gray-800 dark:text-white/90">
                                                {{ $bus->bus_number }} has no driver
                                            </p>
                                            <p class="text-theme-xs mt-0.5 text-gray-500 dark:text-gray-400">
                                                {{ $bus->school?->name ?? 'No school' }}
                                            </p>
                                        </div>
                                    </div>
                                @empty
                                    @if ($expiringBuses->isEmpty() && $suspendedDrivers->isEmpty())
                                        <p class="text-theme-sm text-gray-500 dark:text-gray-400">Everything looks
                                            good.</p>
                                    @endif
                                @endforelse
                ...
                <div class="mt-6 grid grid-cols-12 gap-4 md:gap-6">
                    <div class="col-span-12 xl:col-span-7">
                        <div
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Schools</h3>
                                    <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">
                                        {{ $totalSchools }} schools registered
                                    </p>
                                </div>
                                <a href="{{ route('schools.index') }}"
                                    class="text-theme-sm font-medium text-brand-500 hover:text-brand-600">View all</a>
                                ...
                                <table class="w-full text-left text-sm">
                                    <thead
                                        ...
                                        <tr>
                                            <th class="pb-3 font-medium">School</th>
                                            <th class="pb-3 font-medium">Status</th>
                                            <th class="pb-3 font-medium">Buses</th>
                                            <th class="pb-3 font-medium">Students</th>
                                            <th class="pb-3 text-right font-medium">Routes</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                        @forelse ($schools as $entry)
                                            ...
                                            <tr class="text-gray-700 dark:text-gray-200">
                                                ...
                                                <td class="py-3.5 text-theme-sm">{{ $entry['buses'] }}</td>
                                                <td class="py-3.5 text-theme-sm">{{ $entry['students'] }}</td>
                                                <td class="py-3.5 text-right text-theme-sm">{{ $entry['routes'] }}
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                    ...
                    <div class="col-span-12 xl:col-span-5">
                        <div
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">System Snapshot
                                    </h3>
                                    <p class="mt-1 text-theme-sm text-gray-500 dark:text-gray-400">
                            ...
                            <div class="mt-5 min-h-0 flex-1 overflow-y-auto custom-scrollbar pr-1">
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="rounded-xl bg-gray-50 p-4 dark:bg-white/[0.03]">
                                        <span class="text-theme-sm text-gray-500 dark:text-gray-400">Active
                                            Routes</span>
                                        <h4 class="mt-1 text-2xl font-bold text-gray-800 dark:text-white/90">
                                            {{ $activeRoutes }} <span
                                                class="text-theme-sm font-normal text-gray-400">/
                                                {{ $totalRoutes }}</span></h4>
                                        <a href="{{ route('routes.index') }}"
                                            class="text-theme-xs mt-1 inline-block font-medium text-brand-500 hover:text-brand-600">View
                                            all</a>
                                    </div>
                                    <div class="rounded-xl bg-gray-50 p-4 dark:bg-white/[0.03]">
                                        <span class="text-theme-sm text-gray-500 dark:text-gray-400">Route Stops</span>
                                        <h4 class="mt-1 text-2xl font-bold text-gray-800 dark:text-white/90">
                                            {{ $totalRouteStops }}</h4>
                                        <a href="{{ route('routes.index') }}"
                                            class="text-theme-xs mt-1 inline-block font-medium text-brand-500 hover:text-brand-600">View
                                    ...
                                    <div class="rounded-xl bg-gray-50 p-4 dark:bg-white/[0.03]">
                                        <span class="text-theme-sm text-gray-500 dark:text-gray-400">Parents</span>
                                        <h4 class="mt-1 text-2xl font-bold text-gray-800 dark:text-white/90">
                                            {{ $totalParents }}</h4>
                                    ...
                                    <div class="rounded-xl bg-gray-50 p-4 dark:bg-white/[0.03]">
                                        <span class="text-theme-sm text-gray-500 dark:text-gray-400">School
                                            Admins</span>
                                        <h4 class="mt-1 text-2xl font-bold text-gray-800 dark:text-white/90">
                                            {{ $totalSchoolAdmins }}</h4>
                                        <a href="{{ route('school-admins.index') }}"
                                            class="text-theme-xs mt-1 inline-block font-medium text-brand-500 hover:text-brand-600">View
                                            all</a>
                                    </div>
                                    <div class="rounded-xl bg-gray-50 p-4 dark:bg-white/[0.03]">
                                        <span class="text-theme-sm text-gray-500 dark:text-gray-400">Buses
                                            Online</span>
                                        <h4 class="mt-1 text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                                            {{ $onlineBuses }}</h4>
                                    </div>
                                    <div class="rounded-xl bg-gray-50 p-4 dark:bg-white/[0.03]">
                                        <span class="text-theme-sm text-gray-500 dark:text-gray-400">Checked In
                                            Today</span>
                                        <h4 class="mt-1 text-2xl font-bold text-gray-800 dark:text-white/90">
                                            {{ $todayCheckedIn }} <span
                                                class="text-theme-sm font-normal text-gray-400">/
                                                {{ $todayAttendance }}</span></h4>
                                ...
                                <div class="mt-5 rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                                    <h4 class="text-theme-sm font-semibold text-gray-800 dark:text-white/90">Quick
                                        Actions</h4>
                                    <div class="mt-3 grid grid-cols-2 gap-3">
                                        <a href="{{ route('schools.create') }}"
                                            class="rounded-lg bg-brand-500 px-4 py-2.5 text-center text-theme-sm font-medium text-white hover:bg-brand-600">
                                            Add School
                                        </a>
                                        <a href="{{ route('buses.create') }}"
                                            class="rounded-lg border border-gray-300 px-4 py-2.5 text-center text-theme-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/[0.03]">
                                            Add Bus
                                        </a>
